...
void FullSystem::linearizeAll_Reductor(bool fixLinearization, std::vector<PointFrameResidual*>* toRemove,
                                       int min, int max, Vec10* stats, int tid)
{
	for(int k=min;k<max;k++) {
		PointFrameResidual* r = activeResiduals[k];

        // 是否是stereo能量项的优化。按理stereo里得有一个权重。
...
{
	if(frameHessians.size() < 2) return 0;   // size == 0, 1
...
//    std::cout << "optimize resInA: " << ef->resInA << std::endl;
